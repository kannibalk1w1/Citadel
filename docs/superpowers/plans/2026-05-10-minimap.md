...
**Goal:** Fix broken click navigation, add drag-to-pan, and add item type colour coding to the Minimap component.

**Architecture:** Single file replacement — `Minimap.tsx` gains a `mapTransformRef` shared between the render effect and event handlers, correct inverse-transform navigation math, native DOM drag listeners on `window`, and a per-type colour lookup. No new files, no store changes.

**Tech Stack:** React, TypeScript, HTML Canvas 2D API, Zustand (read-only via store selectors)
...
| File | Change |
|---|---|
| `src/renderer/ui/Minimap.tsx` | Full replacement |

---

### Task 1: Replace Minimap.tsx

**Files:**
- Modify: `src/renderer/ui/Minimap.tsx`

- [ ] **Step 1: Read the current file**

Read `src/renderer/ui/Minimap.tsx` in full so you understand what you're replacing.

- [ ] **Step 2: Replace the entire file**

Write exactly this to `src/renderer/ui/Minimap.tsx`:
...
export function Minimap(): React.ReactElement | null {
  ...
  useEffect(() => {
    ...
    // Share transform with event handlers
  ...
  useEffect(() => {
    return () => {
      // Cleanup any in-flight drag listeners on unmount
...
```
git add src/renderer/ui/Minimap.tsx
git commit -m "fix: minimap drag-to-pan, correct navigation math, item type colours"
```
