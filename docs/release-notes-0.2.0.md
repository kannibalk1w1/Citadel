...
On the other eight — video, audio, YouTube, 3D, GIFs, text, comparisons, code —
...
keybindings — so if you rebind something, the tooltip follows. They were typed
out by hand before, which is why they'd drifted.
...
tool and moves. Connect, Link and Tag are left alone — a click means something
specific in those.

**Notes open for typing straight away.** Place one and the cursor is already in
it, the same as a text item — no double-click. And the tool steps back to Select
...
recent build — never. It's worth explaining, because the fix changes how it
behaves everywhere.
...
isn't allowed to see the pointer while its window is ignoring the mouse — so
that reading froze at wherever your cursor happened to be when you turned the
mode on, and the panel could never notice you approaching it. It sat there
looking like a button and did nothing.
...
around 154 MB. Official builds were never affected — they build from a clean
checkout — but anyone building from source was getting a very silly file.
