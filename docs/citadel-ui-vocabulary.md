...
Atmosphere is everything else: visual style names, mascot effects, canvas
ambience, and prose in documentation.
...
- **Board appearance**: Ambience (Still / Motes / Fog), Presence, Vignette,
  Glow, Floor.
- **Mascot effects** — `rune-seal`, `lightning-out`, `ember-drift` and the rest.
  Internal identifiers; the user never reads them.
...
3. Is it a style name, an effect, or prose? Flavour is welcome.
4. Does the same thing already have a name somewhere else in the app? Use that
   ...
   the rail, for the same button and the same shortcut.
