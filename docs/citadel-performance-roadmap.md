...
2. Add a keyboard-first command palette for actions, navigation, and common creation commands. It must use the existing ActionName/keybind system rather than a parallel command layer.
3. Add a clean first-run onboarding flow that explains the board, importing, creating notes/code cards, search, and the window overlay controls. It must be skippable and never block opening an existing project.
