...
- Grouping, ungrouping, and ordering dispatch batch `ITEM_STYLE` events, including neighbours displaced by an order swap. Explicitly cleared group IDs use JSON-safe `null` in event payloads and normalize back to absent IDs during replay.
- Each archive import extracts into its own unique sibling `_citadel_assets-*` directory. Failed imports remove only their own extraction, never another open project's assets.
- Fitted exports use CSS-space dimensions and rotated item bounds; a transient render filter excludes unselected Konva items. Every capture freezes the bitmap before asynchronous media work. Connections reuse live SVG geometry in the export painter, and poster decoding is deduplicated, visible-only, and concurrency-limited to four.
- Canvas viewport culling handles rotated bounds and window resizing. Audio visualizers reuse buffers and drawing contexts and release their media graph on unmount or source replacement.
- Transcription retains its originating board and source, but ignores results after project replacement, deletion, or relinking. An in-memory project-session revision also prevents export restoration from changing a newly opened project with reused IDs.
- Production dependency fixes include PDF.js 6, using its maintained legacy main/worker builds for Electron 29 compatibility rather than handwritten API shims. Keep a real Electron PDF-import regression alongside its unit tests because browser API compatibility cannot be proved by bundling alone.
- `npm audit --omit=dev` reports zero advisories after this refresh. The full audit still reports 27 across the Electron/build/test dependency tree (including two critical); this is not a clean full-stack security audit. A coordinated Electron, electron-builder, Vite, and Vitest upgrade remains separate work and needs packaged Windows/Linux validation.
- `docs/research-workflow.md` supplies a practical walkthrough. `npm run release:itch` prepares four separate desktop downloads and checksums from existing release artifacts; it does not build, upload, or publish them. The live itch listing still requires an authenticated editor session.
...
- The browser demo is a separate static Vite build (`npm run build:demo`), not an Electron package in an iframe. It reuses the renderer with a narrow browser IPC bridge, opens the bundled showcase automatically, and intentionally keeps project files/native window controls out of scope. Its canvas edits are session-only and Reset demo reloads the showcase.
...
- Performance work should preserve clarity and responsiveness. Motion is welcome when it is restrained, purposeful, and clean (closer to Refflow than game UI); do not add ornamental chrome, fantasy/gothic framing, themed cursors, or effect queues.
...
- Index marks have a hard 24-item budget and are selected through `visibleIndexMarkItems`: only active-board results currently mounted in the viewport can receive a mark. They are quiet static outlines, not rune marks or a pulse. Saved trails remain deliberately deferred until real multi-board archive use proves they would help.
...
- The command palette (`ui/palette/`, `palette:toggle`, Ctrl/Cmd+K) is a lens over the action system, not a registry beside it. Rows come from `resolver.registeredActions()` — so a command exists only if some feature registered a handler for it — labels come from `actionLabels`, and shortcuts from `resolver.bindingsFor()`, which reads the same merged map `resolve` uses so a user override is displayed rather than the default. Running a row calls `resolver.dispatch`, so undo, recording and tool-mode rules apply exactly as from the keyboard. Board jumps are the one non-action row: board switching is not an `ActionName`, so those call the same `setActiveBoard` the tabs and navigator use, which pushes no canvas event by design. Intentional exclusions: no direct item creation (placing an item is a canvas interaction, and a palette row would have to duplicate Toolbar logic rather than dispatch an action — the tool-mode actions are offered instead), and the palette does not list itself. It opens through the normal resolver path, so App's existing editable-field guard already keeps it from firing while typing.
- First-run orientation is a small non-modal `Onboarding` card. It explains the board, opening/importing work, notes and code cards, archive search/commands, and the safe overlay controls. It can be dismissed at any time, records `ui.onboardingComplete`, and uses existing actions for its buttons. It never intercepts opening an existing project.
- Clean motion uses the shared `--motion-enter` / `--motion-toast` tokens only where state changes need orientation: transient panels settle in once, confirmation toasts arrive once, buttons acknowledge press, and DOM-layer media/code items lift while dragged. It has no loops or fantasy effects, and the existing reduced-motion rule collapses all of it. New motion should opt into these classes or add a similarly bounded interaction reason rather than animate by default.
- Keybind overrides now live in `%APPDATA%/Citadel/keybinds.json`, separate from boards and general settings. `KeybindResolver.setOverrides` updates the same live map used for dispatch and palette display; the settings panel captures a shortcut, rejects collisions, allows intentional unbinding, and can reset an action to its default. Saving also rebuilds Electron's native menu from the same file, so a menu accelerator cannot remain stuck on an old binding. Invalid/stale on-disk entries are discarded when loading. The click-through global escape hatch stays fixed at Ctrl+Alt+C for recoverability.
- Click-through makes the whole main window ignore the mouse, and shows the Stop control as a **separate always-on-top window** (`main/stopWindow.ts`, rendered by `renderer/stop.html`). Electron can only ignore the mouse for a whole window, so an in-window panel had to be simulated: the panel reported its rect and main polled `screen.getCursorScreenPoint()` every 50ms, handing the mouse back as the cursor entered it. That cannot work on Wayland — no client may read the pointer while its window is ignoring mouse events, so the reading stayed frozen at wherever the pointer was when the mode turned on and the panel was never reachable. Confirmed on KDE Wayland: 283 consecutive samples with zero pointer movement while the mouse was being swept, with X's own `xdotool` equally stuck. A second window that never ignores the mouse needs no cursor reading at all, so there is one behaviour on all three platforms and no poll, margin heuristic or coordinate conversion. The `Ctrl+Alt+C` global shortcut remains the guaranteed exit, and click-through is still refused outright if that shortcut cannot be registered.
...
- Image and PDF export captures the Konva stage canvas, which by construction cannot see the DOM overlay, so every DOM-layer item is absent from exports. Code cards are now repainted onto a copy of the captured bitmap by `export/codeCardExport.ts` — background, header, terminal dots, language, gutter, and syntax-coloured lines, using `--code-*` resolved through `canvasColors` because a 2D context cannot read CSS variables. Layout constants and the tokenizer are shared with the live card via `CODE_CARD_LAYOUT` and `tokenizeSnippet`, so the still cannot drift from the card it represents. Deliberate limits: the Copy button is a control and is not drawn; lines are clipped rather than wrapped, matching the card's `white-space: pre` — the card scrolls to the rest, a still has nowhere to scroll to — with a `…` on any cut line; overflow past the card height becomes a `… +N more lines` marker that is given its own row. Content is scale-invariant — the whole card scales together, so the same lines export at any board zoom. Video, YouTube, audio and 3D items are now covered too, through the same composition pass (`export/domLayerExport.ts`).
- DOM-media export takes the still the preview pipeline already cached where one exists: `video` and `model3d` draw their poster/preview frame from `assetMetadata.thumbnailPath`, letterboxed, with a corner type badge so a still frame is never mistaken for a plain image. Everything else draws a neutral placeholder carrying its type and identity (filename, or the YouTube video id) so a gap is never silent. Nothing live is captured — no `<video>`, no `<webview>`, no Three renderer, no playback controls. Two types have no safe static source at all and always place-hold: YouTube, because the only still is a network fetch and Citadel renders offline; and audio, because the waveform is an AnalyserNode reading live playback and is flat whenever nothing is playing. A poster that fails to decode degrades to the placeholder rather than failing the export. Boards with no visible DOM-layer items skip poster decoding; the bitmap is still copied so later redraws cannot alter the export.
- DOM-layer items are rotated in the export the way `DOMItem` rotates them live: about the top-left corner, applied before the clip so the clip turns with the item. Konva items always rotated, so without this a board mixing the two exported inconsistently — a turned code card, video or 3D item came out square-on and overlapped the neighbours it had been placed around. `domLayerTransform.ts` holds the one transform both painters use; it sits outside them because each is imported by `domLayerExport`, so neither can own it without a cycle.
