...
- Click-through keeps one interactive panel (`ui/shell/ClickThroughPanel.tsx`). Electron can only ignore the mouse for a whole window, so the region is simulated: the panel reports its rect through `window:setInteractiveRegion` (geometry only — `windowMode` in main stays the single source of the mode itself), and while click-through is on main polls `screen.getCursorScreenPoint()` every 100ms and flips `setIgnoreMouseEvents` as the cursor enters and leaves that rect. The documented `setIgnoreMouseEvents(true, { forward: true })` hover trick was rejected because `forward` is macOS/Windows only, so on Linux no pointer event reaches the renderer at all; polling behaves identically on all three, so it is the only path rather than a fallback. Geometry converts through `webContents.getZoomFactor()`, since UI scale changes CSS pixel size without moving the window. Intentional behaviour: with no region reported the window stays fully click-through, so the failure direction never silently swallows clicks; the panel withdraws its rect on unmount and when the mode ends; the `Ctrl+Alt+C` global shortcut remains the guaranteed exit and stops the watch too. Known limit: for up to one poll interval after the cursor leaves the panel the whole window is still interactive, so a very fast click just outside it can land on the app instead of passing through.
