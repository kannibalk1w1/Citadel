...
test('a capture keeps its source, location, and connection', async () => {
  ...
  await page.keyboard.press('Enter')
  await expect(page.getByText('Image region: left, top, width, height (%) — optional:')).toBeVisible()
  await page.keyboard.type('10, 20, 60, 40')
  await page.keyboard.press('Enter')
  ...
  await expect(page.getByText('Image region: 10%, 20%, 60%, 40%')).toBeVisible()
